...
Below we initialize a vector of 100 paramaeter values. We know that proportions can potentially take an infinite number of values between 0 and 1. Yet, we can approximate the result with just 100 values.
...
We can plot this is grid as an approximation of our prior distribution. Notice that the resolution is worse that in the analytical case. But it should be fine for most of our practical cases.
...
For each of the parameter values, we are comuting its likelihood. 
...
```{r}
data_grid = data_grid %>% 
  mutate(likelihood = dbinom(3, 10, prob = p_grid)) %>% 
...
To sample from the posterior distribution that we obtained, we will use a function called `sample()`.
...
And plot a histogram or density plot (both will do a fine job here). Notice how close it is to the analytical posterior.
...
Sometimes you will want to test whether posterior distribution supports your hypothesis (or whether it allows rejection of some null hypothesis). This is not strictly Bayesian, and it is only for people who cannot live without p-values. 
...
- with samples from your posterior distribution find the 89% HPDI
...
And now we are plotting frequencies of each of future potential outcomes. 
...
But other values in total have higher plausibility. 
